...
export class TinymceEditorComponent implements OnInit, AfterViewInit {
  ...
  editorConfig: any = {
    ...
    external_plugins: {
      ...
      editable: '/assets/tinymce/plugins/editable/plugin.js',
      form: '/assets/tinymce/plugins/form/plugin.js',
      checkbox: '/assets/tinymce/plugins/checkbox/plugin.js'
    },
    // 自定义 css 文件 @ref https://www.tiny.cloud/docs/demo/format-custom/
    content_css: [
      '/assets/tinymce/css/custom.css'
    ],
    // menubar: '',
    // 定义快捷栏的操作, | 用来分隔显示
      ...
      if (el.tagName === 'IMG') {
        ...
        if (node && node.classList.contains('checkbox')) {
          if (node.classList.contains('checkbox-active')) {
            ...
            node.firstElementChild.src = '/assets/tinymce/img/checkbox_unchecked.svg';
          } else {
            // 未选中 -> 选中
            ...
            // node.firstChild.style.cursor = 'pointer';
            node.firstElementChild.src = '/assets/tinymce/img/checkbox_checked.svg';
          }
        }
      }
    });
